import { ObjectValidator, ErrorsParams, SchemaRow } from './ObjectValidator.header'
...
export default class JsonValidator implements ObjectValidator{
  ...
  public async validate(body: Record<string, any>): Promise<ErrorsParams | null>  {
    ...
    Object.keys(this.schema).map( field => {
      ...
      const { type, optional, label, missingMessage, invalidMessage } = this.schema[field] 
